 ...
 * @LastEditTime: 2020-05-24 21:03:53
...
public class Human_YeastSub38NTest extends GraphFileReaderSpec {
    ...
    void readSimilarityMatrix() throws IOException {
        String humanPath = "src/test/java/resources/TestModule/HGATestData/HumanYeastSub38N/HumanNet.txt";
        String yeastPath = "src/test/java/resources/TestModule/HGATestData/Human-YeastSub38N/net-38n.txt";
        yeast38 = readAdjList(yeastPath);
        human9141 = readAdjList(humanPath);

        stopwatch.start();
        String simPath = "src/test/java/resources/TestModule/HGATestData/Human-YeastSub38N/table.Yeast-Human.txt";
        simList = readAdjList(simPath);
        // 4476*6144
        System.out.println("Elapsed :"+ stopwatch.elapsedTime()+" second.");   
        // Elapsed :15.229 second.  
        // reassure it contains all nodes for two graphs
        // yeast
        assertTrue(simList.getRowSet().containsAll(yeast38.getAllNodes()));
        assertTrue(simList.getColSet().containsAll(human9141.getAllNodes()));
