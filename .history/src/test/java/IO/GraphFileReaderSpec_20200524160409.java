...
public class GraphFileReaderSpec {
    ...
    protected void assertNodeNumber(int expected,String filePath) throws IOException {
        if(reader == null)
        reader = new GraphFileReader();
        AdjList graph = reader.readToAdjL(filePath);
        assertEquals(expected,graph.getAllNodes().size(), "nodes number is not right.");
    }

    protected void assertEdgeNumber(int expected,String filePath) throws IOException {
        reader = new GraphFileReader();
        AdjList graph = reader.readToAdjL(filePath);
        assertEquals(expected,graph.getAllEdges().size(), "nodes number is not right.");
    }
